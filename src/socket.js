...
module.exports = function (socket, io, username_socket_pair, all_rooms) {
	...
	socket.on('disconnecting', function () {
		let room_id = '';
		// If the the client was connected to a room, find the room_id
		if (socket.rooms) {
		...
		// If a room_id is found, emit an event that the client has disconnected
		if (room_id) {
	...
	socket.on('create_room', async function (username, callback) {
		...
		// Ensure that it is not a duplicate of an existing room_id
		while (exist) {
		...
		// Send back the joined room_id to the client
		callback(room_id);
	});
